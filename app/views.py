...
class ProductDetailView(View):
    def get(self, request, pk):
        ...
        # Retrieve related products based on the category of the current product
...
def topwear(request, data=None):
    ...
    elif data == 'SweatShirt' or data == 'SweatShirt3':
        topwear = Product.objects.all().filter(category='TW').filter(brand=data)
    elif data == 'above':
        topwear = Product.objects.all().filter(
            category='TW').filter(discounted_price__gte=50000)
    elif data == 'below':
        topwear = Product.objects.all().filter(
            category='TW').filter(discounted_price__lt=50000)
...
def bottomwear(request, data=None):
    if data == None:
        bottomwear = Product.objects.all().filter(category='BW')
    elif data == 'SweatShirt' or data == 'SweatShirt3':
        bottomwear = Product.objects.all().filter(category='BW').filter(brand=data)
    elif data == 'above':
        bottomwear = Product.objects.all().filter(
            category='BW').filter(discounted_price__gte=50000)
    elif data == 'below':
        bottomwear = Product.objects.all().filter(
            category='BW').filter(discounted_price__lt=50000)
